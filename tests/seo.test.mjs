...
const {outputFiles}=await build({entryPoints:['src/lib/seo.ts'],bundle:true,write:false,format:'esm',platform:'node'});
const code=outputFiles[0].text;
const seo=await import('data:text/javascript;base64,'+Buffer.from(code).toString('base64'));
...
test('canonical URLs normalize aliases, query strings and trailing slashes',()=>{
 assert.equal(seo.canonicalUrl('/faq/?utm_source=test'),seo.SITE_URL+'/sss');
...
test('FAQ graph matches all visible answers; business and service types are valid',()=>{
 const graph=seo.pageGraph(seo.seoPages.find(p=>p.path==='/sss'),faqs)['@graph'];
 const faq=graph.find(n=>n['@type']==='FAQPage');
 assert.deepEqual(faq.mainEntity.map(n=>[n.name,n.acceptedAnswer.text]),faqs.map(f=>[f.question,f.answer]));
 assert.equal(graph.filter(n=>n['@id'].endsWith('#organization')).length,1);
 assert.equal(graph[0]['@type'],'FoodEstablishment');
 assert.equal(seo.pageGraph(seo.seoPages.find(p=>p.service))['@graph'].at(-1)['@type'],'Service');
...
test('built pages have distinct initial metadata, one canonical and one schema graph',async()=>{
 ...
 const faqHTML=await readFile('dist/sss/index.html','utf8');
 assert.equal((faqHTML.match(/<details>/g)||[]).length,faqs.length);
});
test('sitemap excludes placeholders and routing preserves CMS categories with a 404 fallback',async()=>{
 const sitemap=await readFile('dist/sitemap.xml','utf8');
 assert.equal((sitemap.match(/<loc>/g)||[]).length,seo.seoPages.length);
 assert.ok(!sitemap.includes('lastmod'));
 for(const page of seo.seoPages) assert.ok(sitemap.includes('<loc>'+seo.canonicalUrl(page.path)+'</loc>'));
 const rules=await readFile('dist/_redirects','utf8');
 assert.ok(rules.includes('/faq /sss 301!'));
 assert.ok(rules.includes('/catering-and-more/* /category.html 200'));
...
test('all published blog URLs and full articles survive the merge',async()=>{
 ...
 for(const post of seo.blogPosts){
  ...
  assert.ok(page);
  const article=seo.pageGraph(page)['@graph'].find(n=>n['@type']==='BlogPosting');
  assert.equal(article.headline,post.title);
  assert.equal(article.datePublished,post.created_at);
  const html=await readFile('dist/blog/'+post.slug+'/index.html','utf8');
